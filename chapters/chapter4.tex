...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    step/.style={draw, rounded corners, align=center, minimum width=2.7cm, minimum height=0.9cm, fill=gray!8},
    ...
    \node[step] (enroll) {Real-user\\enrollment};
    \node[step, right=of enroll] (template) {Template\\creation};
    \node[step, right=of template] (test) {Genuine / impostor\\coding tests};
    \node[step, right=of test] (verify) {Verify / identify\\monitor};
    \node[step, below=1.0cm of verify] (finalize) {Finalize and\\archive};
    \node[data, left=of finalize] (records) {Evaluator and\\keystroke DBs};
    \node[step, left=of records] (metrics) {Metric snapshots\\and thesis evidence};
