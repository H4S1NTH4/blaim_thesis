...
\subsection{Situational Variability in Keystroke Data}
\label{subsec:situational-variability}

Keystroke dynamics is useful because typing rhythm contains stable personal structure, but it is not fixed in the same way as a physical biometric. A student's timing pattern can shift when the assessment situation changes. Stress, time pressure, fatigue, injury, keyboard familiarity, device type, language familiarity, emotional state, and cognitive load can all influence dwell time, flight time, error correction, pause length, and rhythm consistency.

Several studies support this concern. Vizer et al. showed that cognitive and physical stress can be detected from keystroke and linguistic features, which means stress can measurably change normal typing behaviour \cite{vizer2009stress}. Epp et al. demonstrated that emotional states such as nervousness, confidence, relaxation, sadness, tiredness, anger, and excitement can be inferred from keystroke dynamics \cite{epp2011emotional}. Lee et al. further examined emotion-induced variation in keystroke duration, latency, and typing accuracy \cite{lee2014emotion}. Kang and Cho showed that authentication performance depends on input device and text length, with different performance across PC keyboard, soft keyboard, and touch keyboard contexts \cite{kang2015inputdevices}. More recent work by Mac Donald et al. explicitly studied stress as a factor in keystroke dynamics and reported that time constraint, physical limitation, and knowledge constraint can alter typing patterns compared with a normal baseline \cite{macdonald2026stress}.

This variability is especially important for programming assessments. Coding is not simple transcription. A student may pause to reason about an algorithm, hesitate when syntax is unfamiliar, speed up when copying a known pattern, slow down under time pressure, or delete repeatedly while debugging. Edwards et al. show that keystroke data behaves differently across written-language and programming-language contexts \cite{edwards2020keystroke}. Therefore, BLAIM treats keystroke rhythm as probabilistic evidence rather than a single fixed signature. The system must model normal variation, preserve the context of the session, and avoid treating every rhythm shift as misconduct.
...
Keystroke authentication has been studied in online examination settings because it can operate without cameras and without interrupting the learner \cite{mattsson2020keystroke}. Alshehri et al. frame continuous keystroke authentication as an iterative process that repeatedly verifies a user during an active assessment rather than only after typing has finished \cite{alshehri2018iterative}. Lu et al. and Yang et al. further show how deep sequence models can support free-text continuous authentication, including short keystroke windows in uncontrolled settings \cite{lu2020continuous,yang2021tkca}. Senarath et al. re-evaluated keystroke dynamics for continuous authentication and identified the practical value of repeated checking, while also noting sensitivity to environment and device changes \cite{senarath2023reevaluating}. These findings support the BLAIM design decision to perform both direct verification and rolling monitoring, and to expose confidence and risk instead of treating every mismatch as misconduct.
...
Arakawa et al. studied in situ identification of self-regulated learning struggles in programming assignments and showed that trace behaviour can reveal meaningful struggle moments \cite{arakawa2021situ}. Dong et al. used student trace logs to identify meaningful progress and struggle during programming problem solving \cite{dong2021trace}. Edwards et al. directly studied keystroke data from programming contexts and showed that programming language and context influence keystroke patterns and model transferability \cite{edwards2020keystroke}. Later work by Edwards, Hart, and Shrestha introduced public CS1 keystroke datasets and emphasized both the value of fine-grained coding logs and the deidentification challenges created by keystroke-level data \cite{edwards2023csedm}. The IDE Action Log dataset released by Leinonen and Hellas also demonstrates that keystroke-level IDE traces can support fine-grained analysis of introductory programming behaviour \cite{leinonen2022ide}. These studies support the idea that deletion bursts, long pauses, rewriting, and non-linear progress can be educationally meaningful.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{3.0cm}p{2.4cm}p{2.5cm}p{2.7cm}p{3.0cm}@{}}
    ...
    Free-text continuous authentication \cite{alshehri2018iterative,lu2020continuous,yang2021tkca} & Yes & No & No & Adapts continuous authentication to source-code production and instructor evidence \\
    ...
    Programming keystroke and trace analytics \cite{arakawa2021situ,dong2021trace,edwards2020keystroke,edwards2023csedm} & No & Yes & Research-oriented & Adds biometric identity continuity to process analytics \\
...
The reviewed literature shows strong work in separate areas, but a practical gap remains. Keystroke biometrics can authenticate users continuously, but the dominant body of work is based on passwords, free text, general computer use, or broad online examination settings rather than source-code production. Programming trace analytics can explain learning behaviour during coding, but usually lacks biometric identity continuity. Remote proctoring can provide oversight, but raises privacy and equity concerns and does not directly model how code is produced.

This creates a clear novelty position for BLAIM. The underexplored space is not keystroke dynamics in general, and it is not programming trace analytics in general. The underexplored space is the combined use of live coding keystrokes for continuous student authentication and interpretable behaviour analysis within a deployable programming-assessment service. BLAIM addresses this gap by combining continuous keystroke authentication and behaviour analysis in one microservice architecture designed for programming assessment workflows.
