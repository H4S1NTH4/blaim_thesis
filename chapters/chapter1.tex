...
\textbf{First, authentication and learning analytics are usually separated.} Keystroke biometric systems often focus on accept/reject identity decisions, while learning analytics systems focus on student progress, struggle, and intervention. In online programming assessments, the same keystroke stream can support both purposes, but many systems duplicate collection pipelines or ignore one side of the evidence.
...
The gap addressed in this work is therefore not simply ``build another keystroke classifier''. The research gap is the lack of an integrated, interpretable, deployable module that combines continuous keystroke identity monitoring with behaviour analysis for programming assessment workflows.
...
The novelty of this research is the integration of two normally separate concerns: continuous identity assurance and process-aware behaviour analysis. The contribution is practical and applied: BLAIM does not only compute a biometric match. It also records how the typing session unfolded and presents evidence that can help instructors decide whether a case requires academic integrity review, formative support, or normal acceptance.
